...
This will cause ```make``` to create ```sapi/phpdbg/phpdbg```, subsequently ```make install``` will copy the ```phpdbg``` binary to the same path as the ```php``` binary.
...
*phpdbg cannot be built independently of the source code of PHP.*
